...
class EnviaAlteracaoMovimentacao:
    ...
    def mes_por_escrito(self, mes_arq):
        try:
            if mes_arq == "01":
                mes_certo = "Janeiro"
            elif mes_arq == "02":
    ...
    def desmembra_datas(self, data_mov):
        dia_relatorio = data_mov.strftime("%d")
        mes_relatorio = data_mov.strftime("%m")
        ano_relatorio = data_mov.strftime("%Y")
        data_str = f"{dia_relatorio}/{mes_relatorio}/{ano_relatorio}"
    
        mes_escrito = self.mes_por_escrito(mes_relatorio)
    
        return dia_relatorio, mes_escrito, ano_relatorio, data_str
    
    def datas_relatorio(self):
        try:
            cursor = conecta.cursor()
            cursor.execute(f"SELECT MAX(data_mov) FROM envia_mov;")
            max_data_mov = cursor.fetchall()
            max_data_mo = max_data_mov[0]
            max_mov = max_data_mo[0]
    
            cursor = conecta.cursor()
            cursor.execute(f"SELECT data_envia, hora_envia FROM envia_mov where data_mov = '{max_mov}';")
            select_com = cursor.fetchall()
            max_env, hora_envia = select_com[0]
    
            combined = datetime.combine(max_env, hora_envia)
    
            return max_mov, max_env, hora_envia, combined
    
        except Exception as e:
            trata_excecao(e)
            raise
    
    def envia_email_com_anexo(self, data_mov):
        try:
            ...
            body = f"{saudacao}\n\n" \
                   f"Houve modificações na movimentação do estoque no dia {data_str}.\n\n\n\n" \
                   f"{msg_final}"
    
            msg.attach(MIMEText(body, 'plain'))
    
            nome_arquivo = f'Mov {dia_relatorio} de {mes_escrito}.xlsx'
            caminho_arquivo = fr'C:\Users\Anderson\PycharmProjects\robo_boby\{nome_arquivo}'
    ...
    def envia_email_sem_saldo(self, produtos_sem_saldo):
        try:
            ...
            body = f"{saudacao}\n\nHouve problemas com a movimentação alterada em alguns materiais\n" \
                   f"que ficaram com saldo negativo.\n\n" \
    ...
    def salvar_envio_altera(self, ides_mov_altera):
        try:
            ...
            cursor = conecta.cursor()
            cursor.execute(f"Insert into ENVIA_ALTERACAO (ID, ID_MOV, DATA_ENVIA, HORA_ENVIA) "
                           f"values (GEN_ID(GEN_ENVIA_ALTERACAO_ID,1), {ides_mov_altera}, '{data_hoje}', "
                           f"'{horario}');")
            conecta.commit()
    
            print("salvo no banco altera")
    
        except Exception as e:
            trata_excecao(e)
            raise
    
    def salvar_envio_exclui(self, ides_mov_exclui):
        try:
            data_hoje = date.today()
            ...
            cursor = conecta.cursor()
            cursor.execute(f"Insert into ENVIA_EXCLUSAO (ID, ID_MOV, DATA_ENVIA, HORA_ENVIA) "
                           f"values (GEN_ID(GEN_ENVIA_EXCLUSAO_ID,1), {ides_mov_exclui}, '{data_hoje}', "
                           f"'{horario}');")
            conecta.commit()
    
            print("salvo no banco exclui")
    
        except Exception as e:
            trata_excecao(e)
            raise
    
    def excluir_arquivo(self, data_mov):
        try:
            dia_relatorio, mes_escrito, ano_relatorio, data_str = self.desmembra_datas(data_mov)

            nome_arquivos = fr'C:\Users\Anderson\PycharmProjects\robo_boby\Mov {dia_relatorio} de {mes_escrito}.xlsx'
    ...
    def sql_movimentacao_alt(self, envia_mov, ontem):
        try:
            print(envia_mov, ontem)
            cursor = conecta.cursor()
            cursor.execute(f"SELECT m.id, m.criacao, m.data FROM movimentacao m "
                           f"INNER JOIN localestoque ON (m.localestoque = localestoque.id) "
                           f"WHERE m.data <= '{envia_mov}' and m.criacao >= '{ontem}' and m.criacao >= '{ontem}' "
                           f"and localestoque.id IN (1, 2) "
                           f"order by m.data, m.id;")
            select_movimentacao_alt = cursor.fetchall()

            return select_movimentacao_alt

        except Exception as e:
            trata_excecao(e)
            raise
    
    def sql_movimentacao_exc(self, envia_mov, ontem):
        try:
            cursor = conecta.cursor()
            cursor.execute(f"SELECT m.id, m.criacao, m.data FROM mov_excluido m "
                           f"INNER JOIN localestoque ON (m.localestoque = localestoque.id) "
                           f"WHERE m.data <= '{envia_mov}' and m.criacao >= '{ontem}' and m.criacao >= '{ontem}' "
                           f"and localestoque.id IN (1, 2) "
                           f"order by m.data, m.id;")
            select_movimentacao_exc = cursor.fetchall()

            return select_movimentacao_exc
    ...
    def excel_op(self, data, op_int, cod, descr, ref, um, entrada, obs, consumo_os, estrutura, caminho, aba_sheet):
        try:
            ...
            img = drawing.image.Image(fr'C:\Users\Anderson\PycharmProjects\robo_boby\logo.jpg')
            ...
            if itens_sem_id == 0:
                ...
                lanca_dados_mesclado(f'L{linhas_certas3}:M{linhas_certas3}', f'l{linhas_certas3}', "Total Mov.",
                                     12, True)
    ...
    def final_alterados(self):
        try:
            datas_p_altera = []
            ides_p_altera = []
    
            max_mov, max_env, hora_envia, combined = self.datas_relatorio()
    
            select_movimentacao_alt = self.sql_movimentacao_alt(max_mov, combined)
    
            if select_movimentacao_alt:
                for dados_mov in select_movimentacao_alt:
                    print(dados_mov)
                    ides_mov, criacao_mov, emissao_mov = dados_mov
    
                    date_mov = criacao_mov.date()
    
                    if date_mov > emissao_mov and criacao_mov > combined:
                        cursor = conecta.cursor()
                        cursor.execute(f"SELECT * FROM envia_alteracao where id_mov = {ides_mov};")
                        select_envio = cursor.fetchall()
    
                        if not select_envio:
                            print(f"data alteração: {max_mov}, {combined}")
                            print(select_movimentacao_alt)
                            datas_p_altera.append(emissao_mov)
                            ides_p_altera.append(ides_mov)
    
            return datas_p_altera, ides_p_altera
    
        except Exception as e:
            trata_excecao(e)
            raise
    
    def final_excluidos(self):
        try:
            datas_p_excluidos = []
            ides_excluidos = []
            ...
            select_movimentacao_exc = self.sql_movimentacao_exc(max_mov, max_env)
    
            if select_movimentacao_exc:
                for dados_mov in select_movimentacao_exc:
                    ides_mov, criacao_mov, emissao_mov = dados_mov
                    ...
                    if date_mov > emissao_mov and criacao_mov > combined:
                        cursor = conecta.cursor()
                        cursor.execute(f"SELECT * FROM envia_exclusao where id_mov = {ides_mov};")
                        select_envio = cursor.fetchall()
    
                        if not select_envio:
                            print(f"data exclusão: {max_mov}, {max_env}")
                            datas_p_excluidos.append(emissao_mov)
                            ides_excluidos.append(ides_mov)
    
            return datas_p_excluidos, ides_excluidos
    
        except Exception as e:
            trata_excecao(e)
            raise
    
    def final2(self, data_mov):
        try:
            dia_relatorio, mes_escrito, ano_relatorio, data_str = self.desmembra_datas(data_mov)
            ...
            mov_tipo, ops_entradas = self.juntando_mov_por_tipo(data_mov)
            for dados in mov_tipo:
                dat_s, cod_s, des_s, ref_s, um_s, ent_s, sai_s, saldo_s, op_s, cfo_s, loc_s, sol_s, obs_s = dados
                if saldo_s < 0:
                    dad = (
                        dat_s, cod_s, des_s, ref_s, um_s, ent_s, sai_s, saldo_s, op_s, cfo_s, loc_s, sol_s, obs_s)
            ...
            if saldo_negativo > 0:
                ...
                nomis = fr'C:\Users\Anderson\PycharmProjects\robo_boby\Mov {dia_relatorio} de {mes_escrito}.xlsx'
    
                modelo = r'C:\Users\Anderson\PycharmProjects\robo_boby\files\excel\agora.xlsx'
                aba = 'Movimentação'
                ...
                if ops_entradas:
                    for dados_op in ops_entradas:
                        data, cod, descr, ref, um, entrada, saida, saldo, op, cfop, local, solicitante, obs = dados_op
                ...
                self.envia_email_com_anexo(data_mov)
                self.excluir_arquivo(data_mov)
    ...
    def finalizando(self):
        try:
            ...
            datas_p_altera, ides_p_altera = self.final_alterados()
            datas_p_excluidos, ides_excluidos = self.final_excluidos()

            datas_acumuladas = []
            for dat_alt in datas_p_altera:
                datas_acumuladas.append(dat_alt)

            for dat_exc in datas_p_excluidos:
                datas_acumuladas.append(dat_exc)

            lista_datas = remove_repetidos(datas_acumuladas)
            for datinhas in lista_datas:
                self.final2(datinhas)

            for idezinhos_alt in ides_p_altera:
                self.salvar_envio_altera(idezinhos_alt)

            for idezinhos_exc in ides_excluidos:
                self.salvar_envio_exclui(idezinhos_exc)
...
chama_classe = EnviaAlteracaoMovimentacao()
