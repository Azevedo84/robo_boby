...
class ExecutaPlanoPcp:
                        ...
                        if precisa_orcamento == "S":
                            if precisa_desenho == "S":
                                ...
                                for iiitens in itens:
                                    ...
                                    tem_orcamento = self.verifica_orcamento_enviado_banco(cod, ops_dest, vnd_dest)
    ...
    def extrair_ops_pis(self, num_ops):
        if not num_ops:
            return []

        # remove o primeiro "-"
        num_ops = num_ops.lstrip("-").strip()

        # divide pelos "-"
        return [int(op.strip()) for op in num_ops.split("-") if op.strip()]

    def verifica_orcamento_enviado_banco(self, codigo, ops_dest, vnd_dest):
        try:
            msg_ops = ""
            if ops_dest:
                for i in ops_dest:
            ...
            if msg_ops:
                indicador = self.extrair_ops_pis(msg_ops)
                condicoes = " OR ".join([f"NUM_OPS LIKE '%{op}%'" for op in indicador])
            elif msg_pis:
                indicador = self.extrair_ops_pis(msg_pis)
                condicoes = " OR ".join([f"NUM_PIS LIKE '%{pi}%'" for pi in indicador])
            else:
                condicoes = ""
            ...
            if condicoes:
                ...
                tem_orcamento = cursor.fetchone() is not None
            else:
                tem_orcamento = None
                print("MANDER EMAIL URGENTE")
    ...
    def gera_excel_orcamento(self, num_sol, dados_itens):
        try:
            ...
            RAIZ_PROJETO = BASE_DIR.parent.parent  # correto
