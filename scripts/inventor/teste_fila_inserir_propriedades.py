...
class GravarPropriedadeInventor:
    ...
    def worker_fila(self):
        ...
        try:
            ...
            cursor.execute("""
                    SELECT fila.ID_ARQUIVO, fila.NOME_PROPRIEDADE, fila.VALOR_PROPRIEDADE, arq.caminho, arq.NOME_BASE
                    FROM FILA_LANCA_PROPRIEDADE as fila
            ...
            if fila:
                ...
                for id_arquivo, nome_prop, valor_prop, caminho, nome_base in fila:
                    ...
                    try:
                        ...
                        sucesso = self.processar_arquivo(inventor, caminho, nome_prop, valor_prop)

                        if sucesso:
                            cursor.execute("""
                                            DELETE FROM FILA_LANCA_PROPRIEDADE
                                            WHERE ID_ARQUIVO=?
                                        """, (id_arquivo,))
    ...
    def processar_arquivo(self, inventor, caminho, nome_prop, valor_prop):
        try:
            print(f"\n📂 Processando: {caminho}")
            ...
            print(f"{nome_prop} atual:", props.Item(nome_prop).Value)

            props.Item(nome_prop).Value = valor_prop
