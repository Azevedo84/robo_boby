...
class ConferenciaXmlNf:
    ...
    def envia_email_erros_nf(self, erros, msg, pasta, nome_arquivo):
        try:
            saudacao, msg_final, to = self.dados_email()

            subject = f"PRÉ LANÇAMENTO NF – {msg}"
            ...
            print(f'EMAIL COM PROBLEMAS ENVIADO COM SUCESSO!')
    ...
    def verifica_se_e_nfe(self, caminho_xml):
        try:
            tree = ET.parse(caminho_xml)
            root = tree.getroot()

            # verifica se possui a tag infNFe no namespace oficial
    ...
    def inicio_processo(self):
        try:
            arquivos_xml = self.listar_xmls_pasta()
            ...
            for caminho_xml in arquivos_xml:
                print(f"\n🔎 Processando: {os.path.basename(caminho_xml)}")

                if not self.verifica_se_e_nfe(caminho_xml):
                    print("Arquivo não é NF-e válida.")
                    continue

                erros = []

                dados_nf = self.ler_nfe_xml(caminho_xml)
                ...
                pasta = os.path.dirname(caminho_xml)
                ...
                if not dados_fornecedor:
                    ...
                    print(msg)
                    self.envia_email_erros_nf(erros, msg, pasta, nome_arquivo)
                    continue
    ...
    def ler_nfe_xml(self, caminho_xml: str) -> dict[str, Any] | None:
        try:
