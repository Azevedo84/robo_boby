                ...
                for i in ops_abertas:
                    ...
                    if material_faltando:
                        for ii in material_faltando:
                            num_op, cod_pai, descr_pai, cod_e, descr_e, ref_e, um_e, sobras, tipo_e, local_e = ii
                            dados = (num_op, cod_pai, descr_pai, cod_e, descr_e, ref_e, um_e, sobras, local_e)
                            tudo_tudo.append(dados)
    ...
    def verifica_ops_concluidas(self, num_op, id_estrut, cod_pai, descr_pai):
        try:
            ...
            for dados_estrut in select_estrut:
                ...
                id_mat_e, cod_e, descr_e, ref_e, um_e, qtde_e, saldo_prod_e, tipo_e, local_e = dados_estrut
                ...
                if qtde_total_item_op_float != qtde_e_float:
                    sobras = qtde_e_float - qtde_total_item_op_float

                    if saldo_final >= sobras:
                        dados = (num_op, cod_pai, descr_pai, cod_e, descr_e, ref_e, um_e, sobras, tipo_e, local_e)
    ...
    def estrutura_prod_qtde_op(self, num_op, id_estrut):
        try:
            ...
            cursor.execute(f"SELECT estprod.id, prod.codigo, prod.descricao, "
                           ...
                           f"(estprod.quantidade)) AS Qtde, prod.quantidade, tip.tipomaterial, prod.localizacao "
    ...
    def gerar_excel(self, material_faltando):
        try:
            ...
            df = pd.DataFrame(material_faltando, columns=[
                'Nº OP', 'Cód. Pai', 'Descrição Pai', 'Cód.', 'Descrição', 'Referência', 'Um', 'Qtde', 'Local'])
